...
public class RegexHelper {
	...
	/** 숫자 모양에 대한 형식 검사
	 * @param str - 검사할 문자열
	 * @return boolean - 형식이 맞을경우 true, 맞지 않을 경우 false
	 */
	 ...
	 * @param str -검사할 문자열
	 * @return boolean  - 형식메 맞을 경우 true, 맞지않을경우 false
	 */
	...
	public boolean isEmail(String str) {
		boolean result = false;
		if(isValue(str)) {
			result = Pattern.matches("[0-9a-zA-Z]+(.[_a-z0-9-]+)*@(?:\\w+\\.)+\\w+$",
					str);
		}
		return result;
	}
	...
	public boolean isCellphone(String str) {
		boolean result = false;
		if(isValue(str)) {
			result = Pattern.matches("^01(?:0|1|[6-9])(?:\\d{3}|\\d{4})\\d{4}$",
					str);
		}
		return result;
	}
